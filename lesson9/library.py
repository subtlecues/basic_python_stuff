...
def game_outcome():
    ...
    for outcome in games:
        if (user_choice.lower(), ai_decision) in games[outcome]:
            ...
            file.write(
                f'The game was played. You chose {user_choice.lower()}. Computer chose {ai_decision}. Result was '
                f'{outcome}\n')
            file.close()
            return outcome
...
def timer_func(func):
    """
    This function is a decorator func to count elapsed time while playing the game.
    """
    def wrap_func(*args, **kwargs):
        ...
        Returns:
        float containing elapsed time
        """
        t1 = time()
        result = func(*args, **kwargs)
        t2 = time()
        print(f'The game lasted {(t2 - t1):.2f}s!')
        return result
